...
Produces, per zone:
  1. figures/panache_timeseries_{zone}.png  — 4-panel stacked time series
     (plume area, mean SPM, lon centroid, lat centroid) for dynamic and static
     thresholds overlaid.
  2. figures/panache_missing_{zone}.png     — monthly bar chart of missing days
...
for zone in zones:
    ...
    out = os.path.join(proj_dir, "figures", f"panache_timeseries_{zone}.png")
...
for zone in zones:
    ...
    fig.tight_layout()
    out = os.path.join(proj_dir, "figures", f"panache_missing_{zone}.png")
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved {out}")
