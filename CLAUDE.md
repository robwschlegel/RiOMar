...
- [func/figure.py](func/figure.py) — all publication figures (`Figure_1`, `Figure_2`, `Figure_3`/`Figure_3_panels`/`Figure_3_zone_maps`, `Figure_4_S1_timeseries`, `Figure_5_seasonal_analysis`, `Figure_X11_weekly_results`, `Figure_7_driver_rose`, `Figure_8_driver_category`, `Figure_9_gam_partial`, `Figure_S3_seasonal_boxplots`, `Figure_S_daily_flow`)
...
- **Fixed 2026-08-07**: `func/util.R`'s `load_wind_sub()`/`load_wave()`/`load_surface_current()` (called via `multi.R::load_driver()`) used to leave a stray `dim` attribute on their `date` column — harmless for `lm()`, but `readr::write_csv()` correctly rejected it as a matrix column, which was worked around downstream with `date = c(date)`. Root cause traced to `.nc_time_to_date()` (shared by all three, plus `load_ROFI()`): `ncdf4::ncvar_get()` returns a 1-D NetCDF variable as a length-N `array`, not a bare vector, and that `dim` attribute survives the `as.POSIXct()`/`as.Date()` conversion. Fixed at the single source point with `as.vector(ncvar_get(nc, time_var))`; the downstream `date = c(date)` workarounds in `figure.R::Figure_5_seasonal_analysis()` and `compute_seasonal_trend.R` are now unnecessary and have been removed.
