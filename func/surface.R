...
# Set directory
plume_dir <- "output/REGIONAL_PLUME_DETECTION/GULF_OF_LION/SEXTANT/SPM/merged/Standard/PLUME_DETECTION/DAILY"
...
df_pixel <- summarise(df_full,
                      ...
                      count_on = sum(welling == "on", na.rm = TRUE),
                      count_off = sum(welling == "off", na.rm = TRUE),
                      .by = c("lon", "lat")) |> 
...
plot_count <- ggplot(df_pixel, aes(x = lon, y = lat)) +
  ...
  coord_quickmap(xlim = c(2.5, 8.5), ylim = c(41, 44)) +
